export const SITE_NAME = "NPIX";
export const SITE_FULL_NAME = "Nepal Internet Exchange";
export const SITE_TAGLINE = "Connecting Nepal's Digital Ecosystem";
export const SITE_URL = "https://www.npix.net.np";
export const SITE_DESCRIPTION =
  "NPIX is Nepal's carrier-neutral Internet Exchange enabling local traffic exchange among Internet Service Providers, banks, government organizations, educational institutions, and technology companies.";

export const CONTACT_EMAIL = "info@npix.net.np";
export const NOC_EMAIL = "noc@npix.net.np";
export const CONTACT_PHONE = "+977-1-4000000";
export const NOC_PHONE = "+977-1-4000001";
export const OFFICE_ADDRESS =
  "Putalisadak, Kathmandu 44600, Nepal";

export const SOCIAL_LINKS = {
  facebook: "https://facebook.com/npix.net.np",
  twitter: "https://twitter.com/npix_np",
  linkedin: "https://linkedin.com/company/npix-np",
  youtube: "https://youtube.com/@npix-np",
} as const;
